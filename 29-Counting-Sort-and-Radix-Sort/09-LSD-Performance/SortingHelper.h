...
public:
    ...
    static void sortTest(const string& sortname, T arr[], int n)
    {
        ...
        else if(sortname.compare("LSDSort") == 0)
        {
            if constexpr (!std::is_same_v<T, string>) // c++17
            {
                throw std::invalid_argument("typename should be string");
            }
            if(n == 0)
            {
                throw std::invalid_argument("arr can not be empty.");
            }
            LSDSort::sort(arr, arr[0].length(), n);
        }
