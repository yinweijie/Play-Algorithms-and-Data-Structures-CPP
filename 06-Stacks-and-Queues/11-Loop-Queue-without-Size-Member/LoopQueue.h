...
private:
    ...
    friend ostream& operator<<(ostream& os, LoopQueue<T>& queue)
    {
        os << "Queue: size = " << queue.getSize() << " , capacity = " << queue.getCapacity() << endl;
        os << "front [";
...
public:
    ...
    {
        return (tail >= front) ? (tail - front) : (tail + m_capacity - front);
    }
